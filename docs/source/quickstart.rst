...
.. code-block:: bash
   ...
   $ java -Xmx32g -jar target/mhap-0.1.jar -s<fasta/dat from/self file> [-q<fasta/dat to file or directory>] [-f<kmer filter list, must be sorted>]
...
.. code-block:: bash
   ...
   $ java -Xmx32g -jar target/mhap-0.1.jar -p<directory of fasta files> -q <output directory> [-f<kmer filter list, must be sorted>]
