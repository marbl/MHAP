...
public class ReadData
{
	// length of sequences loaded
	 ...
	 public ReadData(String file, String[] fastaSuffix, int kmerSize) throws Exception
	 {
     ...
     while ((line = bf.readLine()) != null) {
        if (line.startsWith(">")) { 
           if (fastaSeq.length() > 0) addMers(new SequenceId(numProcessed++), fastaSeq.toString().toUpperCase(), kmerSize); 
